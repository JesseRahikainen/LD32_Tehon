...
float calcStringRenderWidth( const uint8_t* str, int fontID )
{
	...
	do {
		...
		if( inTag ) {
			if( codepoint == '>' ) inTag = false;
		} else {
			if( codepoint == '<' ) {
				inTag = true;
			} else if( ( codepoint == 0 ) || ( codepoint == LINE_FEED ) ) {
	...
	int fontID, int camFlags, int8_t depth )
{
	...
	// some basic rich text, anything between < and > is ignored, we test each letter, if it matches something then
	//  do something, this is just to get some very basic color switching in
	...
	int camFlags, int8_t depth )
{
	...
	for( size_t i = 0; i < sb_Count( sbStringCodepointBuffer ); ++i ) {
		if( inTag ) {
			switch( sbStringCodepointBuffer[i] ) {
			case '>':
				inTag = false;
				break;
			}
